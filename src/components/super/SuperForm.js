...
import { parsedDate } from '../../helpers/date.helper';
...
const SuperForm = () => {
  ...
  useEffect(() => setInputState(product.selected), [
    // props.editState,
    product.selected,
    setInputState,
  ]);
  ...
  const Submit = () => {
    ...
    if (product.data.some((user) => user.id === inputState.id)) {
      setInputState(INITIAL);
      return dispatch(UpdateAProduct(inputState));
    }
    dispatch(ReproduceAProduct(INITIAL));

    return dispatch(AddAProduct(inputState));
  };
              ...
              <div className="upload__image-wrapper m-auto">
                ...
                <Form.Group style={{ margin: '1em 0em' }}>
                  ...
                  <Form.Field
                    ...
                    label="Temizle"
                    onClick={() => {
                      dispatch(UpdateAProduct(INITIAL));
                      onImageRemoveAll();
                    }}
                  />
                </Form.Group>
