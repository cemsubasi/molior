...
function SuperOrders(props) {
	...
	function setStatus(orderId, id, cartIndex, arg) {
		axiosCall("put", "/order", {
			...
			.then((result) => setInternalStatus(orderId, cartIndex, arg))
			.catch((err) => console.log("error: ", err));
	}
	...
	function findImg(header) {
		let tost = props.state.find((each) => each.productHeader === header);
...
export default connect(mapStateToProps)(SuperOrders);
